...
class WP_GitHub_Updater {
	...
	public function set_defaults() {
		...
		if ( ! isset( $this->config['last_updated'] ) ) {
			...
			$this->config['last_updated'] = $last_updated ?: '';
		}
	 ...
	 *
	 * @since 1.0
	 * @return string|false $date the date
	 */
	public function get_date() {
		$_date = $this->get_github_data();
		if ( ! is_object( $_date ) || empty( $_date->updated_at ) ) {
			return false;
		}
		return date( 'Y-m-d', strtotime( $_date->updated_at ) );
	}
	 ...
	 *
	 * @since 1.0
	 * @return string|false $description the description
	 */
	public function get_description() {
		$_description = $this->get_github_data();
		if ( ! is_object( $_description ) || empty( $_description->description ) ) {
			return false;
		}
